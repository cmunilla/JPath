...
public interface ValueNodes {
    ...
    public abstract class JsonNode extends ValueNode {
		...
		private static final List<JsonNodeFactory> FACTORIES;
		
		static {
			FACTORIES = new ArrayList<JsonNodeFactory>();
			JsonNodeFactory jsonNodeFactory = null;
			ServiceLoader<JsonNodeFactory> loader = ServiceLoader.load(JsonNodeFactory.class);
			Iterator<JsonNodeFactory> iterator =  loader.iterator();
			while(iterator.hasNext()) {
				jsonNodeFactory = iterator.next();
				if(jsonNodeFactory != null) {
					FACTORIES.add(jsonNodeFactory);
				}
			}
		}
		
		public static JsonNode newInstance(CharSequence charSequence) {
			Configuration configuration = Configuration.defaultConfiguration();
			String implementation = configuration.jsonProvider().getCurrentImplementation();
			for(JsonNodeFactory factory : FACTORIES) {
				if(factory.handle(implementation)) {
					return factory.newInstance(charSequence);
				}
			}
			return null;
		}
		
		public static JsonNode newInstance(Object object) {
			Configuration configuration = Configuration.defaultConfiguration();
			String implementation = configuration.jsonProvider().getCurrentImplementation();
			for(JsonNodeFactory factory : FACTORIES) {
				if(factory.handle(implementation)) {
	    ...
	    protected JsonNode(CharSequence charSequence) {
		json = charSequence.toString();
		parsed = false;
	    }

	    protected JsonNode(Object parsedJson) {
		json = parsedJson;
		parsed = true;
	    }
